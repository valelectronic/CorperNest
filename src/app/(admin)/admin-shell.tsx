...
const housingNavItems = [
  ...
  {
    label: "KYC", fullLabel: "KYC Requests", href: "/admin/kyc",
    icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M12 2L4 6v6c0 4.418 3.582 8 8 8s8-3.582 8-8V6L12 2Z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round"/><path d="M9 12l2 2 4-4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>),
  },
  {
    label: "Receipts", fullLabel: "Rent Receipts", href: "/admin/receipts",
  ...
  {
    label: "Requests", fullLabel: "Property Requests", href: "/admin/property-requests",
    icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round"/><path d="M14 2v6h6" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round"/><path d="M9 13h6M9 17h4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg>),
  },
  {
    label: "Bookings", href: "/admin/bookings",
    icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none"><rect x="3" y="4" width="18" height="18" rx="2" stroke="currentColor" strokeWidth="1.8"/><path d="M16 2V6M8 2V6M3 10H21" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg>),
  },
  {
...
const marketNavItems = [
  ...
  {
    label: "Availability", fullLabel: "Availability Requests", href: "/admin/marketplace/availability",
  ...
  return (
    <>
      <div style={{ minHeight: "100dvh", display: "flex", background: "var(--color-bg)" }}>
        ...
        {/* ── SIDEBAR (desktop) ── */}
        <aside className="admin-sidebar" style={{ width: 220, flexShrink: 0, background: "var(--color-header)", display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100dvh" }}>
          <div style={{ padding: "20px 20px 12px", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            <p style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", margin: "6px 0 0", fontFamily: "var(--font-mono)", letterSpacing: "0.08em" }}>ADMIN PANEL</p>
            {sectionToggle(true)}
          </div>
        ...
        {bottomItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}
              ...
              {item.icon}
              <span style={{ fontSize: 9, fontWeight: active ? 700 : 400, fontFamily: "var(--font-heading)", lineHeight: 1 }}>{item.label}</span>
            </Link>
          );
        })}
        {moreItems.length > 0 && (
          <button onClick={() => setMoreOpen((p) => !p)}
            style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, padding: "10px 2px", background: "none", border: "none", cursor: "pointer", color: isMoreActive || moreOpen ? "var(--color-primary)" : "var(--color-text-muted)" }}>
            {moreIcon}
            <span style={{ fontSize: 9, fontWeight: isMoreActive || moreOpen ? 700 : 400, fontFamily: "var(--font-heading)", lineHeight: 1 }}>More</span>
          </button>
        )}
      </nav>
    </>
