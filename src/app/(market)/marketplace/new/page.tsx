...
// Module-scope price cache — survives re-renders, resets on page reload
...
export default function NewListingPage() {
  ...
  // ── Draft save — debounced 600ms, zero DB impact (localStorage only) ──────
  useEffect(() => {
    const t = setTimeout(() => {
      try {
        localStorage.setItem(DRAFT_KEY, JSON.stringify({
          ...
          hasReceipt, delivery, brand, itemAge, sellerPriceNote,
        }));
      } catch { /* silent */ }
    }, 600);
    return () => clearTimeout(t);
  }, [listingType, category, condition, title, description, price, state, lga, landmark, bundleItems, images, hasReceipt, delivery, brand, itemAge, sellerPriceNote]);
  ...
  async function handlePhotoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (images.length + uploadingCount + files.length > maxPhotos) {
      toast.error(`Maximum ${maxPhotos} photos for ${listingType} listings.`);
      return;
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Show summary — seller reviews earnings before final submit
  ...
  async function doSubmit(numPrice: number) {
    ...
    try {
      ...
      const res = await fetch("/api/marketplace/listings/create", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          listingType, title: title.trim(), category, condition,
          description: description.trim(),
          bundleItems: listingType === "bundle" ? cleanBundleItems : [],
          price: numPrice, state, lga, landmark: landmark.trim(),
          images, hasReceipt, delivery, accountNumber, bankCode, accountName,
          sellerPriceNote: sellerPriceNote.trim() || null,
          refPriceMin:       priceResult?.newMin      ?? null,
          refPriceMax:       priceResult?.newMax      ?? null,
          refPriceSource:    priceResult?.priceSource ?? null,
          refPriceContext:   priceResult?.context     ?? null,
          refPriceGoogleUrl: priceResult?.googleUrl   ?? null,
        }),
      });
      const data = await res.json();
  ...
  const canCheckPrice    = title.trim().length >= 3 && !!category && !!condition && images.length > 0;
  const checksRemaining  = MAX_PRICE_CHECKS - priceCheckCount;

  // Price comparison helper — avoids IIFE in JSX
    ...
    <div style={{ minHeight: "100dvh", backgroundColor: "var(--color-bg)", paddingBottom: 40 }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 30, padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, backgroundColor: "var(--color-bg)", borderBottom: "1px solid var(--color-border)" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 11, color: "var(--color-text-muted)", margin: 0 }}>
            Sold via escrow — you get paid after buyer confirms receipt
          </p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "20px 16px" }}>
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {CONDITIONS.map((c) => (
                <button key={c.value} type="button" onClick={() => setCondition(c.value)} style={{
                  ...
                  <div>
                    <p style={{ fontSize: 13, fontWeight: 700, color: condition === c.value ? "var(--color-primary)" : "var(--color-text)", margin: "0 0 2px" }}>
                      {c.label}
                    </p>
          ...
          {/* ── BRAND + AGE (optional, helps price check) ── */}
          ...
          {/* ── PRICE + AI PRICE CHECK ── */}
          <div>
            <label style={labelStyle}>{listingType === "bundle" ? "Bundle price (₦)" : "Your price (₦)"}</label>
            <input type="text" inputMode="numeric" value={price}
              ...
              placeholder="e.g. 8,000" style={inputStyle}
            />
            ...
            <div style={{ marginTop: 10 }}>
              {!canCheckPrice && (
                <p style={{ fontSize: 11, color: "var(--color-text-muted)", margin: 0 }}>
                  Add item name, category, condition and a photo above to enable the market price guide.
                </p>
              )}
