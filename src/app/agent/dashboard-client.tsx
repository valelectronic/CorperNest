    ...
    <div className="rounded-2xl p-4"
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <span className="text-xs font-semibold px-2.5 py-1 rounded-lg shrink-0"
          style={
            isVerified  ? { backgroundColor: "#E8F5E9", color: "#2E7D32" } :
            isScheduled ? { backgroundColor: "#E8F5E9", color: "#2E7D32" } :
                          { backgroundColor: "#FFF8E1", color: "#F59E0B" }
          }>
          {isVerified ? "Visited ✓" : isScheduled ? "Scheduled" : "Awaiting date"}
        </span>
      </div>
      ...
      {isScheduled && booking.agreedDate ? (
        ...
        <div className="rounded-xl px-3 py-2.5 mb-3"
          ...
          <p className="text-xs" style={{ color: "#92400E" }}>
            Client hasn't scheduled a visit date yet.
          </p>
        </div>
      ) : null}
      ...
      <div className="rounded-xl px-3 py-2.5 mb-3"
        ...
        {booking.renterPhone && (
          <a href={`tel:${booking.renterPhone}`}
        ...
        <a href={`mailto:${booking.renterEmail}`}
          style={{ color: "var(--color-text-secondary)", textDecoration: "none", fontSize: 12 }}>
          {booking.renterEmail}
        </a>
      </div>

      {/* ── VERIFY VISIT SECTION ── */}
      {isScheduled && vs.step === "idle" && (
        <button
          onClick={handleSendCode}
          className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold"
          style={{ backgroundColor: "var(--color-primary)", color: "#fff", border: "none", cursor: "pointer", fontFamily: "var(--font-heading)" }}>
      ...
      {isScheduled && vs.step === "sending" && (
        <div className="flex items-center justify-center gap-2 py-3">
          ...
          <span className="text-sm" style={{ color: "var(--color-text-muted)" }}>Sending code to client…</span>
        </div>
      )}
      ...
      {isScheduled && vs.step === "code-shown" && (
        <div className="rounded-xl p-4 space-y-3"
          style={{ backgroundColor: "var(--color-bg)", border: "1px solid var(--color-border)" }}>
          <p className="text-xs font-semibold uppercase" style={{ color: "var(--color-text-muted)", letterSpacing: "0.5px" }}>
            Verification Code
          </p>

          {/* Big code display */}
          <div className="rounded-xl py-4 text-center"
            style={{ backgroundColor: "var(--color-light)", border: "1px solid var(--color-border)" }}>
            <p className="text-2xl font-black tracking-widest"
              style={{ fontFamily: "var(--font-mono)", color: "var(--color-primary)" }}>
              {vs.code}
            </p>
            <p className="text-xs mt-1" style={{ color: "var(--color-text-muted)" }}>
              Code sent to {vs.maskedEmail}
            </p>
          </div>

          <p className="text-xs text-center" style={{ color: "var(--color-text-secondary)", lineHeight: 1.5 }}>
            Ask the client to read the code from their email. If it matches, tap Confirm Visit.
          </p>

          {vs.error && (
            <p className="text-xs text-center" style={{ color: "#E53935" }}>{vs.error}</p>
          )}

          <div className="flex gap-2">
            <button
              onClick={() => setVs({ step: "idle" })}
              className="flex-1 py-2.5 rounded-xl text-sm font-semibold"
              style={{ backgroundColor: "var(--color-bg)", border: "1px solid var(--color-border)", color: "var(--color-text-muted)", cursor: "pointer" }}>
              Cancel
            </button>
            <button
      ...
      {isScheduled && vs.step === "confirming" && (
        <div className="flex items-center justify-center gap-2 py-3">
          <span className="w-4 h-4 rounded-full border-2 border-t-transparent animate-spin"
            style={{ borderColor: "var(--color-primary)", borderTopColor: "transparent" }} />
          <span className="text-sm" style={{ color: "var(--color-text-muted)" }}>Confirming visit…</span>
        </div>
      )}
      ...
      {(isVerified || vs.step === "done") && (
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl"
          ...
          <span className="text-sm font-semibold" style={{ color: "#2E7D32" }}>Visit verified successfully</span>
        </div>
      )}
    </div>
  );
...
export default function AgentDashboardClient({
  ...
  async function handleDelete(listingId: string, mode: "temporary" | "permanent") {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { alert(data.error ?? "Failed. Try again."); setDeletingId(null); return; }
      if (mode === "permanent") {
  ...
  function handleVerified(bookingId: string) {
    // Move verified booking to show verified state
    setBookings((prev) =>
    ...
    <div className="min-h-screen" style={{ backgroundColor: "var(--color-bg)" }}>
      ...
      <header className="sticky top-0 z-50 px-4 py-4" style={{ backgroundColor: "#1B2E1B" }}>
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          <button className="relative w-9 h-9 rounded-full flex items-center justify-center"
            ...
            {bookings.filter((b) => b.status === "pending" || b.status === "scheduled").length > 0 && (
      ...
      <div className="max-w-2xl mx-auto px-4 py-5 space-y-6 pb-10">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { num: bookings.filter((b) => b.status !== "verified" && b.status !== "completed").length, label: "Incoming", color: "#BA7517" },
            ...
            <div className="space-y-2">
              {staleListings.filter((l) => !deletedIds.has(l.id)).map((l) => (
                <div key={l.id} className="flex items-center gap-2">
                  ...
                  <select value={statusMap[l.id] ?? l.status}
            ...
            <div className="space-y-3">
              {bookings.map((b) => (
                <IncomingBookingCard
                  key={b.id}
                  booking={b}
                  onVerified={() => handleVerified(b.id)}
                />
              ))}
            </div>
          )}
              ...
              {visibleListings.map((l) => {
                ...
                return (
                  <div key={l.id} className="rounded-2xl overflow-hidden"
                    style={{ backgroundColor: "var(--color-card)", border: "1px solid var(--color-border)", opacity: isDeleting ? 0.5 : 1, transition: "opacity 0.2s" }}>
                    <div className="relative w-full" style={{ height: "180px" }}>
                      ...
                      <button onClick={() => setSheetListingId(l.id)} disabled={isDeleting}
                    ...
                    <div className="px-4 pt-3 pb-4">
                      ...
                      <div className="flex items-center gap-2">
                        <label className="text-xs shrink-0" style={{ color: "var(--color-text-muted)" }}>Update status:</label>
                        <select value={currentStatus}
                          onChange={(e) => handleStatusUpdate(l.id, e.target.value)}
                          disabled={updatingId === l.id}
                          className="text-xs px-2.5 py-1.5 rounded-lg flex-1 disabled:opacity-50"
                          style={{ border: "1px solid var(--color-border)", backgroundColor: "var(--color-bg)", color: "var(--color-text)", maxWidth: "180px" }}>
                          {STATUS_OPTIONS.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
                        </select>
                        {updatingId === l.id && (
      ...
      {/* ── BOTTOM SHEET ── */}
      <div className="fixed left-0 right-0 bottom-0 z-50 rounded-t-3xl"
        style={{
        ...
        <div className="px-4 py-3 space-y-2">
          <button onClick={() => sheetListingId && handleDelete(sheetListingId, "temporary")}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs mt-0.5" style={{ color: "var(--color-text-muted)" }}>Hidden from clients. You can restore it anytime.</p>
            </div>
          </button>
          <button onClick={() => sheetListingId && handleDelete(sheetListingId, "permanent")}
            className="w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-left"
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs mt-0.5" style={{ color: "#E57373" }}>Removes listing and all photos. Cannot be undone.</p>
            </div>
          </button>
          <button onClick={() => setSheetListingId(null)}
