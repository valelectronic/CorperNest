...
export default function ForgotPinPage() {
  ...
  async function handleSubmit(e: { preventDefault: () => void }) {
    ...
    try {
      ...
      const phoneIsVerified = checkData.phoneNumberVerified;

      // ── Phone first — but only if it was actually proven reachable
      // before. An unverified number was never confirmed to work via SMS,
      // so attempting it again here would likely just waste a send. ──────
      if (method === "phone" && accountPhone && phoneIsVerified) {
        try {
      ...
      } else if (method === "phone" && accountPhone && !phoneIsVerified) {
        setNotice("This phone number hasn't been verified yet, so we're sending the code to your email instead.");
      }
    ...
    >
      ...
      >
        ...
        >
          {(["phone", "email"] as const).map((m) => (
            <button
              key={m}
              type="button"
              onClick={() => { setMethod(m); setIdentifier(""); setError(""); setNotice(""); }}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {notice && !error && (
            <p className="text-sm" style={{ color: "var(--color-primary)" }}>{notice}</p>
          )}
